...
Today the supported AI tools are **Claude Code**, **Codex**, **Cursor**, **OpenCode**, **Antigravity**, **VS Code**, and **Zed**. Behind the scenes, BrowserOS neo writes a single entry named `BrowserClaw` into the AI tool's own MCP config file, pointing at the endpoint URL above. Nothing else changes.

<Note>
The entry really is called `BrowserClaw`, the product's old name, not `BrowserOS neo`. That is what you will see if you open the config file yourself, and it is the name your AI tool knows the browser by.
</Note>
